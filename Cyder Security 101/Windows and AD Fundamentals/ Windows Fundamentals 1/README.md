...
Notes for Blue Team / Forensics:
- Check NTFS for ADS, since malware commonly hides data there. Example PowerShell command to list streams: Get‑Item -Path 'C:\path\to\file' -Stream *.
- Maintain System32 binary integrity (hashing/whitelisting) to detect tampering.
- Monitor for UAC bypass attempts and privilege escalation events via Windows Event Logs (Security, System).
- Inventory installed applications and services (Programs and Features, services.msc) to detect anomalies.
